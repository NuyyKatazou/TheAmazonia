...
(function ($) {
  ...
  // Class changing runs level-up event by default, since each class
  // tracks their own exp. The level gets set to 1 and then level up
  // is called until the correct level is reached. We don't want to
  // run the level up event for this case
  var TH_LevelUpEvent_GameActorChangeClass = Game_Actor.prototype.changeClass
  Game_Actor.prototype.changeClass = function(classId, keepExp) {
    this._isChangingClass = true;
    TH_LevelUpEvent_GameActorChangeClass.call(this, classId, keepExp);
    this._isChangingClass = false;
  }
  ...
  Game_Actor.prototype.canRunLevelUpEvent = function() {
    if (DataManager.isBattleTest() || this._isChangingClass) {
      return false;
    }
    return true;
  }
